...
- **Write the validator** Create a function `uniqueIsbn(path: SchemaPath<string>): void` that calls `validateHttp()` from `@angular/forms/signals`, passing in the `path` and an options object with:
...
- **Wire it up** Call `uniqueIsbn(schemaPath.isbn)` inside the schema function of `BookCreateForm`, right after the `required()`-call for `isbn`.

---

- **Show the pending state** While the request is in flight, `form.isbn().pending()` is `true` — use it in the template to show a "Checking ISBN…" message beneath the ISBN-Input, instead of the usual error list.
